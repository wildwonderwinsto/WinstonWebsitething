        ...
        <div className="w-full min-h-[50vh]">
          <div className="grid grid-cols-2 gap-3 sm:gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 pb-20">
            {movies.map((movie, index) => {
              ...
              return (
                <div ref={isLast ? lastMovieElementRef : null} key={`${movie.id}-${index}`} className="relative">
          ...
          {!loading && movies.length === 0 && (
             <div className="flex flex-col items-center justify-center py-20 text-zinc-500 text-center px-4">
               {debouncedSearch ? <p>No results for "{debouncedSearch}"</p> : (!settings.tmdbApiKey ? <p>Enter API Key in Settings</p> : <p>No movies found.</p>)}
             </div>
          )}
        </div>
      </div>
